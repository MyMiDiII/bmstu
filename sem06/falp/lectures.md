...
goal
  list([1, 2, 3]).
  member(a, [b, a, c, a]). % поиск
  member(X, [b, a, c, a]). % все элементы
```

Нехвостовую можно преобразовать в хвостовую.

Хвостовая рекурсия в прологе?

Рекурсия -- для доказательства знания использование того же знания.

Рекомендация:

для преобразования нехвостовой в хвостовую могут потребоваться дополнительные
аргументы.

cat --- отсечь заведомо нерезультативные ветки.

Отобранные знания система отмечает меткой.

Далее, мб, не про cat???

Считает, что в одной процедуре только одно правило.

fail -- принудительно включает механизм отката, восстанавливается
резольвента, поиск ответа на тот же вопрос, но с помощью другого знания.
...
Список -- одна из возвожностей формировать утверждения.
